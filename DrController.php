...
class DrController extends V2Controller {
  ...
  public function pages() {
    ...
    public function search_text() {
      ...
      $data = self::_cache_search($post_text);
    ...
    public function search_barcode() {
      ...
      $data = self::_cache_search($post_barcode, true);
    ...
    private static function _cache_search($query_name, $one_record = false) {
      $cache_name = "DR_SEARCH_" . $query_name;
      ...
      // barcode or text search and fetch [data, data], data, NULL
        $query_name = preg_replace("/ /", "%20", $query_name);
        $query_url = "https://www.dr.com.tr/search?q=" . $query_name;

        $data = self::_query_search($query_url, $one_record);
  ...
  private static function _cache_top($query_name, $one_record = false) {
    ...
    if (ApplicationCache::exists("$cache_name")) {
      ...
      $data = self::_query_top($query_url, $one_record);
  ...
  private static function _query_search($query_url, $one_record = false) {
  ...
  private static function _query_top($query_url, $one_record = false) {
